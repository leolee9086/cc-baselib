<template>
    ...
    <el-form 
        ...
        >
            ...
            >
            <div slot="label">
                <span class="el-icon-share"  v-if = "当前块解析属性[item]['type']=='块链接'||当前块解析属性[item]['type']=='block-link'"><span>{{当前块解析属性[item]['label']||item.slice(7)}}</span></span>
                <span class="el-icon-link" v-if = "当前块解析属性[item]['type']=='超链接'||当前块解析属性[item]['type']=='link'"><span>{{当前块解析属性[item]['label']||item.slice(7)}}</span></span>
                <span class="el-icon-edit-outline" v-if = "当前块解析属性[item]['type']=='原始文本'||当前块解析属性[item]['type']=='rawstring'"><span>{{当前块解析属性[item]['label']||item.slice(7)}}</span></span>
                <span class="el-icon-house" v-if = "当前块解析属性[item]['type']=='文本'||当前块解析属性[item]['type']=='string'"><span>{{当前块解析属性[item]['label']||item.slice(7)}}</span></span>
                <span class="el-icon-folder-opened" v-if = "当前块解析属性[item]['type']=='附件'||当前块解析属性[item]['type']=='asset'"><span>{{当前块解析属性[item]['label']||item.slice(7)}}</span></span>
