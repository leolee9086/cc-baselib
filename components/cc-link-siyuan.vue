<template>
    <span data-ref="链接id" v-on:mouseover="打开浮窗($event)" v-on:mouseout="鼠标在链接内=false" v-on:mousemove="移动浮窗($event)">
    ...
    module.exports ={
        ...
        methods:{
            移动浮窗: function($event){
                ...
                if(panel){
                 that.开始移动浮窗=false
                let 挂件自身元素 = that.$root.挂件自身元素
                let 挂件坐标 =that.获取元素视图坐标(挂件自身元素)
                that["X"]=$event.clientX+挂件坐标.X
                that["Y"]=$event.clientY+挂件坐标.Y   
                panel.style.top=that.Y+"px"
                panel.style.left=that.X+"px"}
                }
                }
            },
            获取元素绝对坐标(element){
            ...
            获取元素视图坐标(element){
                let that =this
                var scrollTop = that.获取文档元素(that.$root.挂件自身元素).scrollTop;
                var scrollLeft =that.获取文档元素(that.$root.挂件自身元素).scrollLeft;
            ...
            打开浮窗: function($event){
                ...
                // console.log(临时目标)
                    ...
                    点击事件.initMouseEvent('mouseover', true, false, window.parent, 1, 100, 100, 100,100, false, false, false, false, 0, null);
